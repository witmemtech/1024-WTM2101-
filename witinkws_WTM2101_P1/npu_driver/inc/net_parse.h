...
typedef  enum EventType_ {
    ...
    CUSTOM_SUMNORM = 15,
    CUSTOM_GROUPNORM = 16,
    CUSTOM_BATCH_NORM = 17,
    CUSTOM_MUL_FLOAT_SCALE = 18,
    EVENT_TYPE_MMAX = 19,
} EventType_T;
...
typedef struct OptCopyEvent {
    ...
    CHAR_INT_UNION_T total_len;
    CHAR_INT_UNION_T groupNum;
    CHAR_INT_UNION_T layerID;
    GroupMoveType_T moveType;
    EventType_T type;
}OptCopyEvent_T;
#endif
...
#ifdef USE_EVENT_BATCH_NORMALIZATION
typedef struct BatchNorm_{
    CHAR_INT_UNION_T src_addr;
    SramType_T        src_type;
    EventDataType_t   src_data_type;
    CHAR_INT_UNION_T dst_addr;
    SramType_T        dst_type;
    EventDataType_t   dst_data_type;
    CHAR_INT_UNION_T channel;
    CHAR_FLOAT_UNION_T scale_in;
    CHAR_FLOAT_UNION_T scale_out;
    CHAR_INT_UNION_T dim_size;
    int input_shape[4];
    CHAR_INT_UNION_T axis;
    CHAR_INT_UNION_T gamma_offset;
    CHAR_INT_UNION_T beta_offset;
...
//normalization
#if USE_EVENT_CUSTOM_NORMALIZATION
typedef enum{
    NORM_SOFTMAX = 1,
    NORM_SUM     = 2,
} witmemNormType_T;
...
typedef struct {
    ...
    CHAR_FLOAT_UNION_T scale_in;
    CHAR_FLOAT_UNION_T scale_out;
    witmemNormType_T norm_type;
}NormEvent_T;
#endif

#if USE_EVENT_GROUP_NORM
typedef struct {
    CHAR_INT_UNION_T src_addr;
    SramType_T       src_type;
    EventDataType_t  src_data_type;
    ...
    CHAR_INT_UNION_T input_H;
    CHAR_INT_UNION_T input_C;
    CHAR_INT_UNION_T num_groups;
    CHAR_INT_UNION_T num_channels;
    CHAR_FLOAT_UNION_T scale_out;
    float            *alpha;
    float            *beta;
}GroupNormEvent_T;
...
#if USE_EVENT_MUL_FLOAT_SCALE
typedef struct {
    CHAR_INT_UNION_T src_addr;
    CHAR_INT_UNION_T dst_addr;
    EventDataType_t  data_type;
    CHAR_INT_UNION_T len;
    CHAR_FLOAT_UNION_T scale;
    CHAR_INT_UNION_T clip_bit;
}MulFloatScale_T;
#endif
...
typedef struct Net_config_ {
    ...
    groupElement_T *flowGroupList;
    int flowGroupListLen;
    short currentGroupId;
    short currentGroupRunTimes;
} Net_config_T; 
...
Sparse_info_T *witin_get_sparseObj(Compute_flow_T *flow, int index);
...
// get run info special in condition: group num > 1
void genRunInfo(Net_config_T *netObj, int initFlow, int maxFlow, groupElement_T **runInfoList, int *runInfoLen);
void freeRunInfo(groupElement_T **runInfoList);
